    ...
    PausableUpgradeable
{
    ...
    uint8 public constant TOTAL_VESTING_MONTHS = 48;
    uint8 public vestingMonth;
    ...
    function initialize(
        ...
        initialSupplies = [2, 1, 1, 1, 1];

        maxSupplies = [700, 500, 80, 100, 300];
    }
    ...
    function mintBatch(address to) public onlyOwner {
        ...
        if (initialMintTime == 0) {
            vestingMonth = 1;
            initialMintTime = currentTimestamp;
        }

        // Calculate months passed since last vesting
        uint32 secondsPassed = currentTimestamp - lastVestedTime;
        uint8 monthsPassed = uint8(secondsPassed / SECONDS_IN_A_MONTH);

        require(
            monthsPassed > 0,
            "Minting can be performed only after at least a month has passed"
        );

        uint256[] memory ids = new uint256[](5);
        uint256[] memory amounts = new uint256[](5);
        uint8 remainingVestingMonths = TOTAL_VESTING_MONTHS - vestingMonth + 1;

        for (uint i = 0; i < 5; i++) {
            ids[i] = i;
            uint256 monthlyAmount = (maxSupplies[i] -
                runeToken.totalSupply(i)) / remainingVestingMonths;
            amounts[i] = monthlyAmount * monthsPassed;
        }
        lastVestedTime =
            currentTimestamp -
            (secondsPassed % SECONDS_IN_A_MONTH);
        vestingMonth += monthsPassed;

        require(
            vestingMonth <= TOTAL_VESTING_MONTHS,
            "Minting has been completed for the 4 years"
        );
    ...
    function rollMaterialType(
        ...
        for (uint i = 0; i < 5; i++) {
            if (roll < rollThresholds[i]) {
                for (uint j = 0; j < 5; j++) {
                    id = (i + j) % 5;
                    if (counters[id] < initialSupplies[id]) {
                        counters[id]++;
                        return id;
                    }
                }
